...
const AddListingVariation = props => {
  const classes = useStyles();
  const { data } = props.location.state;
  const [addListingVariationError, setAddListingVariationError] = React.useState({
    status: false,
    message: {
      title: '',
      body: ''
  ...
  const onSubmit = e => {
    ...
    const newVariation = state;

    newVariation.parent_listing_id = data.store_id;
    newVariation.supplier = data.supplier;
    newVariation.store = data.store;
    newVariation.is_variant = '1';
    newVariation.has_variations = '0';

    // before submitting the new listing to the DB -> need to make sure that the repricer formula values are valid numbers
    for (const [key, value] of Object.entries(newVariation.local_refactor_settings)) {
      if (isNaN(value) || value === '') {
        validNumbers = false;
      } else {
        if (typeof value === 'string') {
          newVariation.local_refactor_settings[key] = parseFloat(value);
        }
      }
    }
    if (validNumbers) {
      ipcRenderer.send('add-listing-variation', newVariation);
    }
  };

  React.useEffect(() => {
    ipcRenderer.on('add-listing-variation', (event, result) => {
      const { history } = props;
      if (result.status === 'error') {
        if (result.type === 'reached-account-limit') {
          setAddListingVariationError({
            status: true,
            message: {
        ...
        store.addNotification({
          title: 'Success',
          message: `Listing variation '${result.listing.item_name}' has been successfully added!`,
          type: 'success',
          insert: 'bottom',
    ...
    return function cleanup() {
      ipcRenderer.removeAllListeners('add-listing-variation');
    };
  }, []);
    ...
    <Container maxWidth="md" style={{ marginTop: 80, marginBottom: 20 }}>
      ...
      <Paper elevation={0}>
        ...
        >
          ...
          Add a variation to {data.item_name}
        </Typography>

        {addListingVariationError.status && (
          <Paper className={classes.errorPaper}>
            <Typography variant="h5" component="h3" className={classes.errorH5}>
              {addListingVariationError.message.title}
            </Typography>
            <Typography component="p" className={classes.errorP}>
              {addListingVariationError.message.body}
            </Typography>
          </Paper>
        )}
...
export default compose(withRouter)(AddListingVariation);
